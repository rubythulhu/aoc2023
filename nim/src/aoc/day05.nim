...
proc initAlmanac(input: string): Almanac =
  ...
  while true:
    ...
    case s.kind:
    of Bored:
      if input.scanp(s.idx, ("seeds:", +(' ', (+{'0'..'9'} -> addDigit(s.num, $_) )) -> s.endSeed(), '\L') -> s.endSeed()):
        # echo "seeds: {s.alm.seeds}".fmt
        continue
      
      elif input.scanp(s.idx, (+{'a' .. 'z'} -> s.map.src.add $_), "-to-", (+{'a' .. 'z'} -> s.map.dst.add $_), " map:", '\L'):
        s.kind = Cartographing
        # echo "starting map: src={s.map.src} dst={s.map.dst}".fmt
        continue
      
    of Cartographing:
      if input.scanp(s.idx, (+{'0' .. '9'}) -> addDigit(s.rule.dst, $_), ' ') and 
         input.scanp(s.idx, (+{'0' .. '9'}) -> addDigit(s.rule.src, $_), ' ') and 
